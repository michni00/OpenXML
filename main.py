...
def simple_heuristic(df):
    ...
    return np.where(df[0] > median_elevation, 1, 2)

def two_simple_models(X, y):
            ...
            for act in activations:
                for opt in optimizers:
                    for lr in learning_rates:
                        print(
                            f'Training model with {nl} layers, {nn} nodes, {act} activation, {opt.__name__} optimizer, and {lr} learning rate')
                        history = train_nn(nl, nn, act, opt, lr, X_scaled, y_encoded)
...
def train_nn(n_layers, n_nodes, activation, optimizer, learning_rate, X_scaled, y_encoded):
